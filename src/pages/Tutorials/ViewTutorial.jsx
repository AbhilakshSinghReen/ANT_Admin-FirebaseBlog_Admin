...
import TutorialPreview from "../../components/Preview/TutorialPreview";
...
import { getTutorialCategoryByDocId } from "../../firebase/services/Firestore Services/tutorialCategories";
import { getTutorialSubcategoryByDocId } from "../../firebase/services/Firestore Services/tutorialSubcategories";
import { getTutorialSeriesByDocId } from "../../firebase/services/Firestore Services/tutorialSeries";
import {
  getTutorialByDocId,
  updateTutorialMediaOnlyByDocId,
  updateTutorialByDocId,
  deleteTutorialByDocId,
} from "../../firebase/services/Firestore Services/tutorialTutorials";
import { getCollaboratorByDocId } from "../../firebase/services/Firestore Services/collaborators";
...
export default function ViewTutorial() {
  ...
  const refresh = async () => {
    ...
    const result = await getTutorialByDocId(documentId);
    ...
    const categoryResult = await getTutorialCategoryByDocId(
      result.tutorialCategoryDocId
    );
    const subcategoryResult = await getTutorialSubcategoryByDocId(
      result.tutorialSubcategoryDocId
    );
    const seriesResult = await getTutorialSeriesByDocId(
      result.tutorialSeriesDocId
    );
    const creatorResult = await getCollaboratorByDocId(user.uid);
    ...
    setOriginalObject(docObject);
    setNewObject(docObject);
  ...
  const onDocumentDeleteClick = async () => {
    if (
      ...
      deleteTutorialByDocId(originalObject.docId, {
...
Are you sure you want to publish?`
  ...
  const updateMediaOnly = async (newMediaArray) => {
    await updateTutorialMediaOnlyByDocId(
      documentId,
  ...
  const reloadPreviewButtonOnClick = () => {
    setPreviewObject({
      ...newObject,
      tutorialCategoryTitle: category.title,
      tutorialSubcategoryTitle: subcategory.title,
      tutorialSeriesTitle: series.title,
    });
  };
  ...
  return (
    <Container className={styles.backgroundContainer}>
      ...
      <MediaUpload
        parentFolder={`media/tutorials/tutorials/${originalObject.docId}/uploaded-media`}
      ...
      <TutorialPreview data={previewObject} />
      <br />
      <br />
