...
import {
  deleteTutorialSeriesByDocId,
  getTutorialSeriesByDocId,
  updateTutorialSeriesByDocId,
} from "../../firebase/services/Firestore Services/tutorialSeries";

import { objectDeepEqual } from "../../helpers/jsHelpers";

export default function ViewTutorialSeries() {
  ...
  const refresh = async () => {
    ...
    const result = await getTutorialSeriesByDocId(documentId);
    if (result === null) {
      setNotFound(true);
    }
    ...
    const result2 = await getTutorialCategoryByDocId(
      result.tutorialCategoryDocId
    );
    setSelectedCategory(result2);

    const result3 = await getTutorialSubcategoryByDocId(
      result.tutorialSubcategoryDocId
    );
    setSelectedSubcategory(result3);
  ...
  useEffect(() => {
    document.title = "View Tutorial Series";
  ...
  useEffect(() => {
    if (newObject?.title) {
      document.title = `${newObject?.title} Tutorial Series`;
    }
  ...
  const onDocumentDeleteClick = async () => {
    if (
      window.confirm(
        `Are you sure you want to delete the tutorial series: "${originalObject.title}"?`
      )
    ) {
      deleteTutorialSeriesByDocId(originalObject.docId, {
  ...
  const edit = async () => {
    ...
    await updateTutorialSeriesByDocId(
      documentId,
      originalObject,
      newDocumentObject,
      {
