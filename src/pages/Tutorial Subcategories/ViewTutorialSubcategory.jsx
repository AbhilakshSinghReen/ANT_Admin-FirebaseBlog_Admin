...
import {
  deleteTutorialSubcategoryByDocId,
  getTutorialSubcategoryByDocId,
  updateTutorialSubcategoryByDocId,
} from "../../firebase/services/Firestore Services/tutorialSubcategories";

import { objectDeepEqual } from "../../helpers/jsHelpers";

export default function ViewTutorialSubcategory() {
  ...
  const refresh = async () => {
    ...
    const result = await getTutorialSubcategoryByDocId(documentId);
    if (result === null) {
      setNotFound(true);
    }
    ...
    const result2 = await getTutorialCategoryByDocId(result.tutorialCategoryDocId);
  ...
  useEffect(() => {
    document.title = "View Tutorial Subcategory";
  ...
  useEffect(() => {
    if (newObject?.title) {
      document.title = `${newObject?.title} Tutorial Subcategory`;
    }
  ...
  const onDocumentDeleteClick = async () => {
    if (
      window.confirm(
        `Are you sure you want to delete the tutorial subcategory: "${originalObject.title}"?`
      )
    ) {
      deleteTutorialSubcategoryByDocId(originalObject.docId, {
  ...
  const edit = async () => {
    ...
    await updateTutorialSubcategoryByDocId(
      documentId,
      originalObject,
      newDocumentObject,
      {
