...
export default function ViewBlogSubCategory() {
  ...
  const refresh = async () => {
    ...
    const result = await getBlogSubCategoryByDocId(documentId);
    if (result === null) {
      setNotFound(true);
    }
    ...
    const result2 = await getBlogCategoryByDocId(result.blogCategoryDocId);
  ...
  const onDocumentDeleteClick = async () => {
    if (
      window.confirm(
        `Are you sure you want to delete the blog subcategory: "${originalObject.title}"?`
      )
    ) {
  ...
  const edit = async () => {
    ...
    await updateBlogSubCategoryByDocId(
      documentId,
      originalObject,
      newDocumentObject,
      {
