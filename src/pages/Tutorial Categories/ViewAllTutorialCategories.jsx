...
import { tutorialsCategorySlugs as collectionSlugs } from "../../constants/slugs";

export default function ViewAllTutorialCategories() {
  ...
  useEffect(() => {
    document.title = "Tutorial Categories";
  ...
  const onDocumentDeleteClick = async (targetDocument) => {
    if (
      window.confirm(
        `Are you sure you want to delete the tutorial category: "${targetDocument.title}"?`
      )
    ) {
      deleteTutorialCategoryByDocId(targetDocument.docId, {
  ...
  return (
    <Container className={styles.backgroundContainer}>
      <div className={styles.viewCollectionHeaderDiv}>
        <h1 className={styles.documentAdderHeading}>All Tutorial Categories</h1>
        <IconButton
          variant="contained"
          color="primary"
