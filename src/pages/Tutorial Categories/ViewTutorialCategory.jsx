...
import {
  getTutorialCategoryByDocId,
  updateTutorialCategoryByDocId,
  deleteTutorialCategoryByDocId,
} from "../../firebase/services/Firestore Services/tutorialCategories";

import { objectDeepEqual } from "../../helpers/jsHelpers";

export default function ViewTutorialCategory() {
  ...
  const refresh = async () => {
    ...
    const result = await getTutorialCategoryByDocId(documentId);
    if (result === null) {
      setNotFound(true);
    }
  ...
  useEffect(() => {
    document.title = "View Tutorial Category";
  ...
  useEffect(() => {
    if (newObject?.title) {
      document.title = `${newObject?.title} Tutorial Category`;
    }
  ...
  const onDocumentDeleteClick = async () => {
    if (
      window.confirm(
        `Are you sure you want to delete the tutorial category: "${originalObject.title}"?`
      )
    ) {
      deleteTutorialCategoryByDocId(originalObject.docId, {
  ...
  const edit = async () => {
    ...
    await updateTutorialCategoryByDocId(
      documentId,
      originalObject,
      newDocumentObject,
      {
