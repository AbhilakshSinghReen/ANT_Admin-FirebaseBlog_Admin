...
import { tutorialsSeriesSlugs as collectionSlugs } from "../../constants/slugs";

export default function ViewAllTutorialSeries() {
  ...
  const refresh = async () => {
    ...
    const result2 = await getAllTutorialSeries();
  ...
  const onDocumentDeleteClick = async (targetDocument) => {
    if (
      window.confirm(
        `Are you sure you want to delete the tutorial series: "${targetDocument.title}"?`
      )
    ) {
      deleteTutorialSeriesByDocId(targetDocument.docId, {
  ...
  return (
    <Container className={styles.backgroundContainer}>
      <div className={styles.viewCollectionHeaderDiv}>
        <h1 className={styles.documentAdderHeading}>All Tutorial Series</h1>
        <IconButton
          variant="contained"
          color="primary"
      ...
      {selectedCategoryDocId === ""
        ...
        : allDocuments.map((document, index) =>
            document.tutorialCategoryDocId === selectedCategoryDocId ? (
              <CollectionDisplayer
                documentName={document.title}
                onView={() => onDocumentViewClick(document.docId)}
                onDelete={() => onDocumentDeleteClick(document)}
