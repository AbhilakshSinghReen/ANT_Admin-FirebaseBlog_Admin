...
import { blogCategorySlugs as collectionSlugs } from "../../constants/slugs";

export default function ViewAllBlogCategories() {
  ...
  const onDocumentDeleteClick = async (targetDocument) => {
    if (
      window.confirm(
        `Are you sure you want to delete the blog category: "${targetDocument.title}"?`
      )
    ) {
      deleteBlogCategoryByDocId(targetDocument.docId, {
  ...
  return (
    <Container className={styles.backgroundContainer}>
      <div className={styles.viewCollectionHeaderDiv}>
        <h1 className={styles.documentAdderHeading}>All Blog Categories</h1>
        <IconButton
          variant="contained"
          color="primary"
